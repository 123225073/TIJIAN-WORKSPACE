...
export function WechatLibrary({account,t}:any){
 const [library,setLibrary]=useState<any>(null),[settings,setSettings]=useState<any>(null),[showSettings,setShowSettings]=useState(false);
 const [input,setInput]=useState(account.url||''),[since,setSince]=useState(()=>monthStart()),[until,setUntil]=useState(()=>localDay()),[keyword,setKeyword]=useState(''),[limit,setLimit]=useState('30');
 const [large,setLarge]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState('');
 const [queryPages,setQueryPages]=useState('3');const pause=useRef(false);
 ...
 const bodyJob=submitted&&!deleted?(bodyJobs.find((j:any)=>j.id===submitted.id)||submitted):bodyJobs[0];
 ...
 const f=view==='run'&&run?run.filters:{since,until,keyword,limit:Number(limit)||30};
 const items=(library?.items||[]).filter((x:any)=>(view!=='run'||!run||run.keys.includes(x.article_key))&&(!f.since&&!f.until||x.published&&(!f.since||x.published>=f.since)&&(!f.until||x.published<=f.until))&&(!f.keyword||x.title.toLowerCase().includes(f.keyword.toLowerCase()))).slice(0,f.limit);
 const allIds=new Set((library?.items||[]).map((x:any)=>x.id));
 const chosen=selected.filter(id=>allIds.has(id));
 const unknown=(library?.items||[]).filter((x:any)=>!x.published).length;
 ...
 async function collectBody(mode='public'){setSubmitting(true);setCollectError('');setNotice('正在提交正文采集…');resultAnchor.current?.scrollIntoView({block:'nearest',behavior:'smooth'});try{const bridge=(window as any).tijianDesktop;const job=mode==='public'&&bridge?.wechatBody?await bridge.wechatBody({action:'start',ids:chosen,benchmark_id:account.id,token:getToken()}):await api('/wechat/collect',{ids:chosen,benchmark_id:account.id,mode,confirmed:mode==='cimidata',confirmed_conversion:mode==='cimidata'});setSubmitted(job);setSelected([]);await t.refresh?.();setNotice('采集任务已开始，关闭此窗口也会继续；进度显示在采集按钮旁。')}catch(e){setCollectError(message(e))}finally{setSubmitting(false)}}
 ...
 return <section className="wechat-library">{paid.modal}
  <div className="wechat-intro"><div><span className="section-number">历史补齐 / 持续订阅</span><h2>公众号文章库</h2><p>目录先存入本地，再按需获取正文。历史查询与订阅共用同一个文章库。</p></div><button onClick={()=>{setShowSettings(!showSettings);void refresh()}}>次幂服务设置</button></div>
  <FreeSubscription key={account.id} account={account} bound={library?.account} t={t} onChange={refresh}/>
  {!settings?.configured&&<div className="local-warning"><strong>可选：连接付费目录来源</strong><p>次幂需要独立开通 API 服务。账号识别、目录查询会按供应商规则计费。</p><CimiLinks/><button onClick={()=>setShowSettings(true)}>填写 AppID / Secret</button></div>}
  {showSettings&&<WechatSettings/>}
  <div className="wechat-steps"><span>01 确认账号</span><span>02 同步与筛选目录</span><span>03 归档与导出</span></div>
  <div className="cimi-account"><label className="field"><span>该博主发布的一篇公众号原文链接</span><input aria-label="次幂公众号原文链接" value={input} disabled={busy} onChange={e=>setInput(e.target.value)} placeholder="https://mp.weixin.qq.com/s/…"/></label>
   ...
   {library?.account&&<div className="discovery-result"><strong><Check size={16}/> {library.account.title}</strong><p>账号标识 {library.account.biz} · {library.account.wxid}</p><small>来源：次幂文章账号接口 · {stamp(library.account.verified_at)}</small></div>}
  </div>
  {library?.account&&<>
   ...
   <div className="button-row"><button className="primary" disabled={busy||(Number(limit)>30&&!large)} onClick={()=>confirmQuery(true)}>按此范围获取目录（计费）</button><button onClick={()=>{setView('library');setSelected([])}}>查看本地文章库（免费）</button></div>
   ...
   {run&&<div className="cimi-coverage"><DeleteRecord item={run} t={t} onDeleted={()=>{setRunId('');setView('library');void refresh()}}/><strong>{run.coverage}</strong><p>本轮已读取 {run.pages} 页。按本次页数上限逐页查询；重启后保留断点。新文章会加入本地库，关键词与日期只筛选结果。</p>{run.error&&<p className="form-error">{run.error}</p>}{!run.done&&<button disabled={busy} onClick={()=>confirmQuery()}>从断点继续查询（计费）</button>}</div>}
   <div className="block-heading"><h3>{view==='run'?'本轮结果':'本地文章库'} · {items.length} 篇</h3><span>目录共 {library.items.length} 篇 / 正文已保存 {library.items.filter((x:any)=>x.status==='body_saved').length} 篇</span></div>
   {unknown>0&&<p className="muted">{unknown} 篇发布日期未知；设置日期条件时排除。</p>}
   <div className="button-row"><button disabled={busy||items.length>100} onClick={()=>setSelected(items.map((x:any)=>x.id))}>选择当前结果（最多100篇）</button><button onClick={()=>setSelected([])}>清空选择</button><span>已选 {chosen.length} 篇</span></div>
   <div className="cimi-articles">{items.map((x:any)=><div className="discovery-entry" key={x.id}><input type="checkbox" disabled={busy||chosen.length>=100&&!chosen.includes(x.id)} checked={chosen.includes(x.id)} onChange={e=>setSelected(e.target.checked?[...chosen,x.id]:chosen.filter(id=>id!==x.id))}/><span><strong>{x.title}</strong><small>{x.published||'日期未知'} · {x.status==='body_saved'?'正文已保存':x.status==='failed'?'正文采集失败':'已发现，正文未采集'}</small>{x.error&&<small className="form-error">{x.error}</small>}{x.status==='body_saved'&&<SavedArticle source={t.get?.(x.source_id)}/>}</span><a href={x.url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} aria-label={'打开原文 '+x.title}><ExternalLink size={16}/></a><DeleteRecord item={x} t={t} onDeleted={()=>{setSelected(v=>v.filter(id=>id!==x.id));void refresh()}}/></div>)}{!items.length&&<p className="inline-empty">当前筛选下还没有文章。查询一页目录后即可勾选归档。</p>}</div>
   <div className="button-row"><button className="primary" disabled={busy||collecting||!chosen.length} onClick={()=>void collectBody()}>{collecting?'正在采集正文…':'采集所选正文（不收次幂费用）'}</button><button disabled={busy||collecting||!chosen.length} onClick={()=>paid.confirm('确认次幂付费正文补采？',`已选择 ${chosen.length} 篇，长链接需先转换（¥0.01/次），再获取正文（¥0.01/次）；每篇最多两次调用，本批参考费用最多约 ¥${(chosen.length*0.02).toFixed(2)}。已缓存的短链接不重复转换。已有且未删除的正文直接复用，不重复付费。不会自动重试失败文章，也不会重新查询目录。`,()=>void collectBody('cimidata'))}>次幂正文补采（计费）</button><button disabled={busy||!chosen.length} onClick={()=>void exportZip()}><Download size={16}/>导出目录与已有正文</button></div>
   <div ref={resultAnchor} className={'collection-feedback '+(collecting?'is-running':'')} role="status" aria-live="polite">{submitting?<><RefreshCw size={17} className="spin"/> 正在提交采集，请稍候…</>:bodyJob?<><strong>{collecting?'正在采集中':['failed','interrupted'].includes(bodyJob.status)?'采集失败':bodyJob.status==='cancelled'?'采集已取消':bodyJob.result?.failed?bodyJob.result?.success?'采集部分成功':'采集失败':'采集成功'}</strong><span>{collecting?bodyJob.progress:`成功 ${bodyJob.result?.success||0} 篇 · 失败 ${bodyJob.result?.failed||0} 篇`}</span>{collecting&&bodyJob.progress?.includes('验证')&&<button onClick={()=>{void (window as any).tijianDesktop?.wechatBody({action:'show',token:getToken()}).catch((e:Error)=>setCollectError(e.message))}}>打开微信验证窗口</button>}{collecting&&<button onClick={()=>void action(()=>api('/jobs/'+bodyJob.id+'/cancel',{}))}>取消采集</button>}</>:<span>请选择文章后采集。免费公开读取与次幂付费补采分别操作。</span>}{collectError&&<p className="form-error">{collectError}</p>}</div>
   <p className="muted">免费采集使用独立浏览器，不导入你的微信登录信息；如在验证页登录或确认身份，仍可能关联账号，无法保证没有封号风险。遇到微信验证会弹出窗口，完成后自动继续；关闭验证窗口会取消本批。网页端公开采集可能直接受限；次幂正文补采按次计费，必须再次确认。查看本地文章库和导出不收费。</p>
   {bodyJob&&<><DeleteRecord item={bodyJob} t={t} onDeleted={()=>setSubmitted(null)}/><CollectionResult job={bodyJob} getSource={t.get}/></>}
   <details open={library.subscription?.enabled||undefined}><summary>手动选择备用：付费定时查询</summary><section className="cimi-subscription"><div className="block-heading"><h3><Bell size={17}/> 定时查询更新（付费）</h3><strong>{library.subscription?.enabled?'已开启':'未开启'}</strong></div><p>这是定时付费查询，不是新文推送；即使没有新文章也可能扣费。工作台运行时定时检查；退出或休眠期间暂停，恢复后继续补漏。首次检查建立基线，旧文章不提醒；新增保存在应用内，即使当时页面未打开也能查看。</p><div className="cimi-filters"><label className="field"><span>检查间隔（分钟）</span><input aria-label="订阅检查间隔" type="number" min="30" max="10080" value={minutes} onChange={e=>setMinutes(e.target.value)}/></label><label className="field"><span>每日自动调用上限</span><input aria-label="订阅每日调用上限" type="number" min="1" max="200" value={daily} onChange={e=>setDaily(e.target.value)}/></label><label className="field"><span>每轮最多目录页</span><input aria-label="订阅每轮目录页" type="number" min="1" max="10" value={pages} onChange={e=>setPages(e.target.value)}/></label></div><p className="muted">每日自动调用次数按当前用户的所有公众号合计；每个订阅会按此上限检查。耗尽后停止自动查询，次日继续。此处不承诺分钟级发现或公众号全量历史。</p>
