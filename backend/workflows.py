...
from . import store as s,jobs,network,upstream,gateway as g
...
def register(app,user,error):
    ...
    def open_task(data:dict,u=Depends(user)):
        ...
        with s.LOCK:
            key=s.digest(str((text,mode,sorted(refs),data.get('profile_id') or '')))
            ...
            if old:return old
            return s.export_object(u['id'],s.put(u['id'],'task',{'title':text[:160],'messages':[],'mode':mode,'source_ids':refs,'profile_id':data.get('profile_id'),'entry_key':key,'created':s.now()}))
    ...
    def trash(id:str,u=Depends(user)):
        ...
        if obj['kind'] in ['wechat_article','source'] and any((j.get('status') in ['running','queued'] or j['id'] in jobs.CANCEL) and (id in j.get('input',{}).get('article_ids',[]) or id in j.get('input',{}).get('source_ids',[])) for j in s.list_(u['id'],'job')):error(409,'资料正在被任务使用，请先等待或取消任务')
        ...
        def run(progress,event):
            ...
            for i,url in enumerate(urls):
                ...
                try:
                    ...
                    obj=import_text({**article,'benchmark_id':benchmark,'source_type':'批量采集'},u)
            ...
            return result
        return jobs.start(u['id'],'批量采集文章',run,{'action':'batch',**data})
