...
const dir=path.resolve('.runtime','agent-ui-'+Date.now());fs.mkdirSync(dir,{recursive:true});
const server=net.createServer();await new Promise(r=>server.listen(0,'127.0.0.1',r));const port=server.address().port;await new Promise(r=>server.close(r));
const service=spawn(path.resolve('.runtime/venv/Scripts/python.exe'),['scripts/agent-ui-fixture.py'],{windowsHide:true,stdio:['ignore','ignore','pipe'],env:{...process.env,TIJIAN_DATA:dir,TIJIAN_PORT:String(port)}});let errors='';service.stderr.on('data',x=>errors+=x);const base='http://127.0.0.1:'+port;
try{
 ...
 const child=spawn(createRequire(import.meta.url)('electron'),['scripts/test-agent-ui-runner.cjs',dir,base],{windowsHide:true,stdio:'inherit'});
